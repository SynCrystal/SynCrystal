...
for cntm = 1:numm
    for cntn = 1:numn
        ...
        ssEnergySpl = reshape(ssEnergySpl,[szSST(1),szSST(2),numel(pos1)*numel(pos2)]);
        ssEnergySplh = zeros(size(ssEnergySpl));
        for cnt = 1:numel(pos1)*numel(pos2)
            ssEnergySplh(:,:,cnt) = abs(fft(ssEnergySpl(:,:,cnt)')')/sqrt(szSST(2));
        end
        %adjMat = AdjMatRotInv(ssEnergySplh,sigma);
        adjMat = AdjMat(ssEnergySplh,sigma);
